...
//        let rightButton = UIBarButtonItem(title: "Skip", style: .plain, target: self, action: #selector(goToMainVC))
...
extension SecondVC {
    ...
    private func makeImageViewIphone() -> UIImageView {
        ...
        imageView.image = UIImage(named: "Главный")
    ...
    private func makeLabel() -> UILabel {
        ...
        let attributeText = NSMutableAttributedString(string: "Якість, Краса, Простота, Еко-ініціатива ")
        ...
        attributeText.apply(color: color, toSubstring: "Якість")
        attributeText.apply(color: color, toSubstring: "Краса")
        attributeText.apply(color: color, toSubstring: "Простота")
        attributeText.apply(color: color, toSubstring: "Еко-ініціатива")
    ...
    private func makeSubtitle() -> UILabel {
        let label = UILabel()
        label.text = "Ми придумали такі ручки, щоб ви могли насолоджуватися кожним записом, адже наша Lamy – найкраща версія"
        label.textAlignment = .center
        label.numberOfLines = 0
    ...
    private func makeConstrainsSubtitle() {
        ...
        NSLayoutConstraint.activate([
            ...
            subtitle.leadingAnchor.constraint(equalTo: viewLayer.safeAreaLayoutGuide.leadingAnchor, constant: 16),
            subtitle.trailingAnchor.constraint(equalTo: viewLayer.safeAreaLayoutGuide.trailingAnchor, constant: -16),
        ])
    }
    ...
    private func makeButtonTralling() -> UIButton {
        ...
        button.addTarget(self, action: #selector(tapButtonTrailing), for: .touchUpInside)
        return button
    }
    ...
    private func makeButtonLeading() -> UIButton {
        ...
        button.addTarget(self, action: #selector(tapButtonLeading), for: .touchUpInside)
    ...
    @objc func tapButtonLeading() {
        navigationController?.popViewController(animated: true)
    }
    
    @objc func tapButtonTrailing() {
        let thirdVC = ThirdVC()
...
//    private func makeButtonLeading() -> UIButton {
//        let button = UIButton()
//        let image = UIImage(systemName: "arrowshape.backward.circle", withConfiguration: UIImage.SymbolConfiguration(pointSize: 50, weight: .medium))
//        button.setImage(image, for: .normal)
//        button.tintColor = color
//
//        return button
